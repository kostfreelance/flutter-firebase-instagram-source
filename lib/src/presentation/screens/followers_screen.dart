...
import 'package:flutter_firebase_instagram/src/internal/modules/followers_module.dart';
...
class FollowersScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    FollowersController controller = Get.put(FollowersModule.controller(userId));
    return Scaffold(
      appBar: AppNavBar(
        leftButton: const AppNavBarBackButton(),
        title: Obx(() =>
          AppNavBarTitle('${controller.followersCount.toString()} Followers')
        )
      ),
      body: Obx(() =>
        _ListBuilder(
          ...
          fetchNextItems: controller.fetchNextFollowers
        )
      )
    );
  }
  ...
  Widget build(BuildContext context) {
    return (items != null) ?
      items!.isNotEmpty ?
        ...
        const NoDataLabel('No Followers Yet') :
